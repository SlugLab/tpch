...
#    queries = ['21.sql']
    ...
    for q in queries:
        ...
        write("%s,%d,%s,%.2f,%.2f,%.2f%%,%d,%d,%d,%d,%d", qq(config), seq, qq(name), base_performance[q], 0,0,0,sum(output_rss)/len(output_rss),sum(output_avgrss)/len(output_avgrss),sum(output_vsz)/len(output_vsz),args.memory)
        output_rss=[]
        output_avgrss=[]
        output_vsz=[]
        ...
        for q in queries:
            ...
            if devpercnt > threshold and alert == 0:
                ...
                if devpercnt < threshold and alert == 1:
                    ...
                    if wait == patience:
                        alert = 0
            write("%s,%d,%s,%.2f,%.2f,%.2f%%,%d,%d,%d,%d,%d", qq(config), seq, qq(name), qtime, dev, devpercnt*100, alert,sum(output_rss)/len(output_rss),sum(output_avgrss)/len(output_avgrss),sum(output_vsz)/len(output_vsz),args.memory)
            output_rss=[]
            output_avgrss=[]
            output_vsz=[]
